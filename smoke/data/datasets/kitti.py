...
class KITTIDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for i, a in enumerate(anns):
            ...
            image = draw_box_3d(image, vertexes_2d)
            
            center = np.mean(vertexes_3d, 0)
            center = np.array([center[0], center[1], center[2], 1.0])
            center1 = np.matmul(P, center)

            P_extend = np.concatenate((P, np.ones((1, 4))), axis=0)
            P_extend[3, :3] = 0.0
            P_extend_inv = np.linalg.inv(P_extend)

            center2 = np.array([center1[0], center1[1], center1[2], 1.0])
            center_re = np.matmul(P_extend_inv, center2)

            center1 = center1[:2] / center1[2]
            center1 = affine_transform(center1, trans_mat)
